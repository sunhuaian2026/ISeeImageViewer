...
> 注意：项目根目录磁盘路径仍是 `~/Documents/projects/claude/ISeeImageViewer/`（保 auto-memory 路径不断），仓库内部全部统一为 Glance。GitHub 仓库已于 2026-05-08 改名为 `sunhuaian2026/Glance`（旧路径 `sunhuaian2026/ISeeImageViewer` GitHub 永久 redirect）。
...
- 每个模块完成后立刻 git commit，commit message 格式：「完成 [模块名]」，然后执行 `git push` 同步到 GitHub（remote: git@github.com:sunhuaian2026/Glance.git）。
- **模块完成后必须同步更新文档**：

...
- [ ] (2026-05-07 / `<pending>` / bugfix · F 键全局) **grid F 键**：grid 模式（无图片选中）→ 按 F → 窗口全屏（traffic lights 隐藏）；再按 F → 退出全屏
- [ ] (2026-05-07 / `<pending>` / bugfix · F 键全局) **preview F 键**：单击 cell 进 preview → 按 F → 窗口全屏；再按 F → 退出全屏
- [ ] (2026-05-07 / `<pending>` / bugfix · F 键全局) **QV F 键回归**（不能破坏）：双击 cell 进 QV → 按 F → 全屏；再按 F → 退出全屏（行为不变，跟修前一致）
- [ ] (2026-05-07 / `<pending>` / bugfix · F 键全局) **F 键不冲突其他快捷键**：grid 按方向键 / Space 仍正常；preview 按 ESC / Space / 方向键仍正常

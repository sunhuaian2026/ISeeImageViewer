...
- [x] (2026-05-08 / `9a202d4`) **README 加下载入口 + 4 张产品截图**：README 顶部下载链接已指 latest release（v1.0.0 已发可用）；assets/screenshots/ 加 4 张 1800x1316 深色 PNG（01-grid / 02-preview / 03-quickviewer / 04-inspector）；README hero 段下方插入 4 节带 caption 截图展示 ✓ 2026-05-08
- [x] (2026-05-08 / `f17fe97`) **GitHub 仓库改名 ISeeImageViewer → Glance**：`gh repo rename Glance --repo sunhuaian2026/ISeeImageViewer` 执行成功；新 URL https://github.com/sunhuaian2026/Glance；旧 URL HTTP 301 → 新 URL HTTP 200 redirect 通畅；本地 `git remote set-url origin git@github.com:sunhuaian2026/Glance.git`；V2 worktree 自动跟随（共享 .git/config）；README/CLAUDE.md 4 处硬编码 URL 同步更新 ✓ 2026-05-08

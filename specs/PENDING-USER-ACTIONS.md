...
- [ ] (2026-05-07 / `<pending>` / bugfix) **toolbar fused · 核心**：启动 app → **直接**单击缩略图 cell 进 ImagePreviewView（**不**经过 QuickViewer）→ 顶部应**直接是 f5 状态**（深色融合到 NSWindow title bar，无浅灰断层）
- [ ] (2026-05-07 / `<pending>` / bugfix) **toolbar fused · 进出 QV 回归**：双击缩略图进 QuickViewer → ESC 退出 → 再单击 cell 进 preview → 仍是融合状态（不要回到 f4 浅灰断层）
- [ ] (2026-05-07 / `<pending>` / bugfix) **toolbar fused · sidebar 列视觉**：左上 `+` 按钮 + sidebar toggle 视觉位置 / 间距不变（codex 标的待人工验证项）
- [ ] (2026-05-07 / `<pending>` / bugfix) **toolbar fused · Traffic light 回归**：进 QV 时 traffic light 隐藏 / 退 QV 时恢复，行为不变（之前 commit a064033 / 45a61f1 / 6da903c 已修过 3 次的回归区域，codex 标的视觉间距人工验证项）
- [ ] (2026-05-07 / `<pending>` / bugfix) **toolbar fused · 外观切换**：切换浅色 / 深色 / 跟随系统 → toolbar 视觉跟着切，无残留

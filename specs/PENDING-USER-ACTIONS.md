...
- [ ] (2026-04-25 / `<pending>`) **预览页**：选中文件夹 → 单击缩略图进入内嵌预览 → 按方向键 ←→ 连续切换 → 切换瞬间不应再出现 loading 转圈（首张可能仍转一下，第二张起命中预加载缓存即时显示）
- [ ] (2026-04-25 / `<pending>`) **预览页**：单击进预览 → 双击进 QuickViewer → Esc 退回预览 → 再用方向键切换 → 仍无转圈（focus 恢复 + 缓存正常工作）
- [ ] (2026-04-25 / `<pending>`) **预览页**：在预览中切换文件夹（侧边栏点另一个） → 不应崩溃；旧缓存清空，新文件夹预览正常
- [ ] (2026-04-25 / `<pending>`) **预览页（回归验证）**：单击/双击/Esc/Space/← →/关闭按钮 全部行为不变；n/m 进度、青绿光晕、底部"双击图片进入全屏查看"提示文案、左右导航气泡按钮 视觉无差

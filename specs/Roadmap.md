...
| e0c418d | 排序后缩略图索引错位：`applySortKey` 未重置 `selectedImageIndex`，旧索引在新数组中指向错误图片；ContentView 未在 images 变化时关闭 QuickViewer。修复：排序前置 `selectedImageIndex = nil`；`onChange(of: images)` 关闭 QuickViewer |

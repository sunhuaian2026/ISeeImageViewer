...
| B 时间分段 sticky header（5 段固定）+ "本周新增" + hover tooltip | 🚧 进行中（B-α 时间分段实装，B-β 本周新增 SF 待开始） | V2.0-beta2 | — | 见下方 Slice B 表 |
...
| Sub | Goal | 状态 | Commit |
|---|---|---|---|
| B-α | TimeBucket（5 段算法）+ SmartFolderGridView LazyVGrid sectioned + sticky pinnedViews + section header view | ✅ 完成 | _本 commit_ |
| B-β | BuiltInSmartFolders 加 thisWeekAdded（规则：managed AND !hidden AND dedupCanonical AND birth_time BETWEEN_DURATION ['-7d','now']）+ sidebar 自动出现 | ⏳ 待开始 | — |
| B-γ | /go 五步收尾 + Roadmap ✅ + tag V2.0-beta2 | ⏳ 待开始 | — |

...
| 待修 P2 | Inspector / ContentView | Inspector 开关时多一道粉色竖线 + 关闭后延迟消失（用户截图 `~/sync/e1.png` / `e2.png`，2026-05-06 测试发现）。已 grep 定位元凶：`ContentView.swift:33-39` 的 `Divider()` 在 `if showInspector { Divider(); ImageInspectorView(...).transition(.move(...)) }` 块中，Divider 没加 .transition 修饰，跟 Inspector 的 `.move(edge: .trailing)` 动画不同步：开 Inspector 时 Divider 立刻出现 + Inspector 滑入（视觉上 Divider 提前到位），关 Inspector 时 Inspector 滑出 + Divider 跟外层 `.animation(DS.Anim.normal, value: showInspector)` 慢渐隐（视觉上 Divider 残留）。粉色色调因 SwiftUI Divider 默认色被 accent / tint 渗透。Bug 2/Bug 3 同根因合并修。**修法方向**（待 plan）：(a) 删 Divider 改用 ImageInspectorView 自带 leading border / (b) Divider 加 `.transition(.move(edge: .trailing))` 跟 Inspector 同步 / (c) 同步动画 + 显式 `DS.Color.separatorColor` 配色绕开粉色。Roadmap 计划 v1.0.1 修。 |

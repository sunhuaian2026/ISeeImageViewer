...
| `<pending>` | V2.0 GA Slice D follow-up #2 — hide 图标扩展到 subfolder（explicit 触发）：用户首测发现 subfolder 单独 hide 时（root visible）该 subfolder 行无图标提示，状态视觉缺口。3cd463c 当时把 scope 锁死在仅 root 层，subfolder 完全没图标 → 不对症。修法：(1) IndexStore 加 `isExplicitlyHidden(rootId:, relativePath:)` API — 不走 path-walk 继承，仅查 row 自己 hide_in_smart_view 字段（root row 用 id + parent_root_id IS NULL，subfolder row 用 parent_root_id + relative_path 精确匹配）；(2) ContentView 加 `explicitlyHidden(rootURL:, nodeURL:)` 闭包传给 sidebar；(3) FolderSidebarView.folderRow 去掉 `isRoot` 限制，对所有节点用 `isExplicitlyHidden` 决定图标显示。规则：仅 row 显式 hide=1 才显图标 — root.hide=1 显（保持 3cd463c 行为）/ subfolder 单独 hide 显（修当前 bug）/ subfolder 继承 root hide 不显（避免 root hide 整树时所有 subfolder 都顶图标的视觉爆炸）。`isEffectivelyHidden` 闭包保留给 contextMenu label 用，语义不变 |

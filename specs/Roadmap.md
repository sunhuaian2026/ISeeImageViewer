...
|------|------|------|--------|----------|------|
| V2 follow-up | sidebar 文件夹右键"刷新" | （待写）| 高 | 无 | 用户在 Finder 对受管文件夹做添加/删除后 grid 不即时显示（V1 mode FolderStore.images 是 selectFolder 时一次性扫描快照，无 FSEvents 监听；V2 mode 虽 Slice G 有 FSEvents，但边界场景或 watcher 未启动时可能漏）。需求：FolderSidebarView 右键菜单加"刷新"项，触发对当前 folder 重新扫盘 + reload images（V1）+ IndexStore re-scan + refreshSelected（V2）。**用户主动报，2026-05-09** |

...
**V2 M1 Slice A → B → D → G → H 累积 ship（V2.0-beta1/2/3/4/5）**
...
- **V2 M1 Slice H（V2.0-beta5，2026-05-09 ship + tag）**：内容去重 SHA256 + cheap-first 粗筛（仅 (file_size, format) 撞的 candidate group 算 SHA256，10k 图典型 ~5-10s）+ Inspector 副本段（同 SHA256 fullPath 列表 + Show in Finder per row）+ orphan duplicate 自动 promote
...
- 下一步主线：Slice I 首次索引进度 UI + 错误处理 + SmartFolderStore enum-state 重构（V2.0 RC + GA）
...
| H.3 | /go 五步 + Roadmap ✅ + tag V2.0-beta5 | ✅ 完成 | _本 commit_ |

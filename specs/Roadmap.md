...
| `<pending>` | V2.0 GA SVG preview / QuickViewer 永卡 spinner（c88c7ae 缩略图修法 follow-up）：用户报缩略图能看了但 preview 和 QV 仍卡 spinner。根因：`ImagePreviewViewModel.load` + `QuickViewerViewModel.load` 主路径都用 `CGImageSourceCreateImageAtIndex`，跟 c88c7ae 之前的 loadThumbnail 同 ImageIO API，对 SVG (vector) return nil → spinner。修法：(1) `ImageGridView.swift` 抽 `loadFullNSImage(url:)` 顶层 helper（SVG 走 `NSImage(contentsOf:)` / 其他走 CGImageSource），preview / QV 4 处主路径 + thumbnail 路径将来都可复用；(2) `ImagePreviewViewModel.load` + `QuickViewerViewModel.load` 主路径改调 helper；(3) prefetch 路径（preview / QV 各一）对 SVG `continue` 跳过 — `prefetchCache` 类型 `[Int: CGImage]` 存不了 vector，SVG 是 KB 级 vector 文件 cache miss 时主路径直 load 不亏。`loadFullNSImage` 抽出后未来加新 codec fallback (HEIF live photo 等) 一处改即可 |

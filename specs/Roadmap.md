...
| <pending> | 首次进 ImagePreviewView 顶部出现一条浅灰横向 toolbar 底色，跟下方 preview 紫黑色 (`appBackground` #121217) 明显断层（用户截图 `~/sync/f4.png` bug / `~/sync/f5.png` 正常）。**触发条件**：启动 app → 单击 cell 进 preview → f4 状态；双击进 QuickViewer → ESC 退出 → 再单击 cell 进 preview → 永久变 f5 状态（融合到 NSWindow title bar 深色，无断层）。文件名 + ⓘ Inspector + 外观切换按钮位置不变（detail 列左 / 右），只是 toolbar chrome 形态切换。失焦/聚焦不触发，排除 macOS focus state quirk。**根因**（codex:rescue gpt-5.5 high 拍）：SwiftUI `NavigationSplitView` 首次渲染时 toolbar 默认 separated/expanded 样式（独立浅灰横条）；`ContentView.swift:97` 的 `.toolbar(quickViewerIndex != nil ? .hidden : .visible, for: .windowToolbar)` 在 QV 进出时触发 hide→visible 转换，invalidate 了 NSToolbar/titlebar 布局缓存，AppKit 重算后 NSWindow 锁定 fused 状态。意外副作用 = 用户期望的状态。**修法**：`Glance/FullScreen/WindowAccessor.swift` 在 makeNSView 异步捕获块 + updateNSView 两处幂等设 `window.toolbarStyle = .unified`。两处都设是因为 SwiftUI 可能在 `view.window` 存在前就 attach representable，单点不够稳。Codex 标的待人工验证项：sidebar toolbar 视觉间距 + traffic light 间距，源码层面无功能冲突证据 |

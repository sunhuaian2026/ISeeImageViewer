...
| D hide toggle 右键菜单（root + 子目录 + 状态继承）+ Inspector source path | 🚧 进行中（D.1 hide toggle 端到端实装） | V2.0-beta3 | — | 见下方 Slice D 表 |
...
| Sub | Goal | 状态 | Commit |
|---|---|---|---|
| D.1 | hide toggle 端到端：IndexStore CRUD（setRootHidden / upsertSubfolderHide / effectiveHidden / folderIdForRootPath）+ SmartFolderQueryBuilder `.hidden` walk SQL（path-length DESC 取最具体 explicit）+ FolderSidebarView contextMenu（root + 子目录两层动态 label）+ ContentView 桥（V1 URL → IndexStore 坐标） | ✅ 完成 | _本 commit_ |
| D.2 | Inspector 来源 path 段 + Show in Finder 按钮 | ✅ 完成 | _本 commit_ |
| D.3 | /go 五步 + Roadmap ✅ + tag V2.0-beta3 | ⏳ 待开始 | — |

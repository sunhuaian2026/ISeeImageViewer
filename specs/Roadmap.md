...
| Sub | Goal | 状态 | Commit |
|---|---|---|---|
| H.1 | SHA256 计算 + dedup pass 后端：ContentHasher.swift / DedupPass.swift（runFullPass + reEvaluateGroup + orphan cleanup）/ IndexStore CRUD（setContentSHA256/setDedupCanonical/resetSHA256AndCanonical/promoteOrphanDuplicates/fetchCandidateGroups/fetchImagesInGroup/fetchImageGroupKey/fetchImageIdByPath/fetchDuplicates/fetchDuplicatesByFullPath）+ FolderStoreIndexBridge handle hooks（registerAndScan/handleCreated/handleModified/handleRemoved/unregister 五处） | ✅ 完成 | _本 commit_ |
| H.2 | Inspector 副本段：ImageInspectorView 加 duplicatesProvider closure + 副本 Section 条件展示（fullPath truncation .middle + Show in Finder per row）+ ContentView 注入 IndexStore.fetchDuplicatesByFullPath | ✅ 完成 | _本 commit_ |
...
| Sub | Goal | 状态 | Commit |
|---|---|---|---|
| I.1 | 首次索引进度 chip：IndexingProgress + IndexingProgressView（Capsule + thickMaterial mirror Slice B chip 风格）+ IndexStoreHolder.progress @Published + bridge.onScanProgress 回调 + ContentView mainContent ZStack 顶层挂 | ✅ 完成 | _本 commit_ |
...
| I.3 | SmartFolderStore enum-state 重构：4 态 .idle/.loading/.loaded/.error 替代 selected/queryResult/isQuerying/lastError 四独立 @Published；computed accessors 兼容旧 view 调用；stale-write guard 走 state pattern match | ✅ 完成 | _本 commit_ |
| I.4 | /go 五步 + Roadmap ✅ + tag v2.0（最终 GA） | ✅ 完成 | _本 commit_ |
